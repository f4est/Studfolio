document.addEventListener('DOMContentLoaded', () => {
    ...
    // Убедимся, что все всплывающие подсказки Bootstrap инициализированы (если они используются)
    const tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
    tooltipTriggerList.map(function (tooltipTriggerEl) {
        return new bootstrap.Tooltip(tooltipTriggerEl);
    });

    // Убедимся, что все выпадающие меню Bootstrap инициализированы (хотя обычно они работают без явной инициализации)
    const dropdownElementList = [].slice.call(document.querySelectorAll('.dropdown-toggle'))
    dropdownElementList.map(function (dropdownToggleEl) {
      return new bootstrap.Dropdown(dropdownToggleEl)
    });

    // Обработка рейтинга в форме
